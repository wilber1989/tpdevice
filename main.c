...
char devsendbuf[1024]={0};
char devrecvbuf[1024]={0};
...
/* 数据发送线程*/
int thread_send(int Client, char* devsendbuf)
{   
    printf("发送消息:%s\n",devsendbuf);
    send(Client, devsendbuf, strlen(devsendbuf), 0);
    memset(devsendbuf,0,1024);
    return 0;
}
